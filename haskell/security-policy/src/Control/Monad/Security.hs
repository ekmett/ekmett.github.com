...
-- | A `SecurityPolicy` is a particular relationship between a closed set of `SecurityLevel`s. 
-- The closure of this set is maintained by limiting the export of the policy to the trusted code base. 
-- A policy is a monad in its own right, because to allow certain forms of lattice extensions later
-- we will want to be able to inject values into it. 
class Monad p => SecurityPolicy p

-- | A `SecurityLevel` is a particular point inside our capability space. You should not export the
-- data constructors for your SecurityLevels, but only export whatever methods you want to allow the
...
-- use this to raise the security level of a value using the supplied `witness` for the `SecurityLattice` relationship
